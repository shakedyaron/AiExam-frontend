...
function ScoreBadge({ pct }: { pct: number | null }) {
  ...
  return (
    <span
      className={`text-xs font-bold px-2 py-0.5 rounded-full border ${cls}`}
    >
      {pct}%
    </span>
  );
...
function getCoachData(avg: number | null, weakCount: number) {
  if (avg === null)
    return {
      emoji: "🚀",
      title: "ברוך הבא לאזור הצמיחה!",
      body: "תתחיל עם המבחן הראשון שלך וקבל ניתוח מותאם אישית.",
  ...
  if (avg >= 85)
    return {
      ...
      body: `ממוצע ${avg}% — אתה ברמה גבוהה. שמור על הקצב ואתגר את עצמך עם נושאים חדשים.`,
  ...
  if (avg >= 70)
    return {
      ...
      body: `ממוצע ${avg}% — עוד ${weakCount > 0 ? `שיפור ב-${weakCount} נושאים חלשים` : "קצת עבודה"} ותגיע ל-90%.`,
  ...
  if (avg >= 50)
    return {
      emoji: "⚠️",
      title: "יש פוטנציאל גדול כאן!",
      body: `ממוצע ${avg}% — זיהינו ${weakCount} נושאים שדורשים תשומת לב. תרגול ממוקד יקפיץ אותך קדימה.`,
      color: "from-yellow-600/15 to-orange-700/10",
      border: "border-yellow-500/25",
    };
  return {
    emoji: "🔥",
    title: "הגיע הזמן להתאמץ!",
    body: `ממוצע ${avg}% — רוב הנושאים דורשים שיפור. תרגול ממוקד על הטעויות שלך יעזור לך לזנק.`,
...
function CoachBanner({
  ...
  return (
    <motion.div
      ...
      className={`rounded-2xl border ${c.border} p-5 mb-5 bg-linear-to-l ${c.color}`}
    >
          ...
          {worstExam && (
            <p className="text-xs text-white/35 mt-1.5">
              ...
              <span className="text-red-400/80 font-medium">
                {worstExam.title.slice(0, 45)}
                {worstExam.title.length > 45 ? "…" : ""}
              </span>
            </p>
          )}
        </div>
...
function MotivationBlock({
  ...
  return (
    <motion.div
      ...
      style={{
        background:
          "linear-gradient(135deg,rgba(124,58,237,0.08),rgba(168,85,247,0.04))",
      }}
    >
      <div className="flex items-center justify-between mb-2">
        <span className="text-xs font-semibold text-violet-300">
          🎯 יעד: 90% —{" "}
          {remaining > 0 ? `עוד ${remaining}% ותגיע!` : "הגעת ליעד!"}
        </span>
        {weakCount > 0 && (
          <span className="text-xs text-white/30">
            {weakCount} נושאים חלשים לשיפור
          </span>
        )}
      </div>
...
function InteractiveChart({
  ...
  data: { pct: number; title: string; date: string }[];
}) {
    ...
    >
      ...
      <div className="flex items-end gap-2 h-24">
        {data.map((d, i) => {
          ...
          const barColor =
            d.pct >= 80 ? "#10b981" : d.pct >= 60 ? "#eab308" : "#ef4444";
          return (
            <div
              key={i}
            ...
            >
              ...
              <span
                className={`text-[10px] transition-colors ${isHovered ? "text-white" : "text-white/25"}`}
              >
                {d.pct}%
              </span>
              <div
              ...
              <span
                className={`text-[9px] transition-colors text-center leading-tight ${isHovered ? "text-white/70" : "text-white/20"}`}
              >
                {d.date}
              </span>
            </div>
            ...
            if (delta > 0)
              return (
                <span className="text-[11px] text-emerald-400">
                  ↑ שיפור של {delta}% מאז ההתחלה 🎉
                </span>
              );
            if (delta < 0)
              return (
                <span className="text-[11px] text-red-400">
                  ↓ ירידה של {Math.abs(delta)}% — בוא נהפוך את זה!
                </span>
              );
            return (
    ...
    >
      ...
      {comparison && (
        <div className="flex items-center gap-2 text-xs mb-2 flex-wrap">
          ...
          <span
            className={`font-bold ${comparison.delta >= 0 ? "text-emerald-400" : "text-red-400"}`}
          >
            {comparison.delta >= 0
              ? `↑ +${comparison.delta}% 🎉`
              : `↓ ${comparison.delta}%`}
      ...
      <div className="flex items-center justify-between text-xs text-white/30 mb-3">
        ...
        <div className="flex items-center gap-2">
          <span
            className={`px-1.5 py-0.5 rounded-lg border text-[10px] ${diffColor[item.difficulty] ?? "text-white/30"}`}
          >
...
export default function HistoryPage({ onStartExam }: Props) {
  ...
  const stats = useMemo(() => {
    ...
    const trend =
      pcts.length >= 2 ? pcts[pcts.length - 1] - pcts[pcts.length - 2] : null;
    ...
    () =>
      history
        ...
        .map((h) => ({
          pct: scorePct(h)!,
          title: h.title,
          date: new Date(h.created_at).toLocaleDateString("he-IL", {
            month: "numeric",
            day: "numeric",
          }),
        })),
  ...
  async function generateSmartExam(item: HistoryItem) {
    ...
    if (hasWrong && item.score?.mcqAnswers) {
      try {
        const exam = await generateFromMistakes({
          examId: item.id,
          numQuestions: 6,
  ...
  const filterPills: { key: Filter; label: string; activeClass: string }[] = [
    {
      key: "all",
      label: "הכל",
      activeClass: "border-violet-500/50 bg-violet-500/15 text-violet-300",
    },
    ...
    { key: "hard", label: "קשה", activeClass: diffColor.hard },
    {
      key: "weak",
      label: "רק חלשים ⚠️",
      activeClass: "border-red-500/50 bg-red-500/15 text-red-300",
    },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#080810] text-white" dir="rtl">
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-8 pt-22 pb-12">
        ...
        >
          ...
          <p className="text-white/40 text-sm mt-0.5">
            מערכת האימון החכמה שלך — נתח, שפר, הצלח
          </p>
        </motion.div>
          ...
          <>
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-5">
              <div className="relative flex-1">
                ...
                />
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-white/25 text-sm">
                  🔍
                </span>
              </div>
                ...
                {history.length === 0 ? (
                  <>
                    ...
                    <div className="text-sm">
                      עדיין אין מבחנים — צור את הראשון!
                    </div>
                  </>
                ) : (
                  "לא נמצאו תוצאות"
      ...
      {/* Fallback toast */}
      <FallbackToast
        visible={fallbackToast}
        onDismiss={() => setFallbackToast(false)}
      />
    </div>
  );
}
